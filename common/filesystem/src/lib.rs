...
pub fn create_with_600_perms<P: AsRef<Path>>(path: P, bytes: &[u8]) -> Result<(), Error> {
    ...
    #[cfg(unix)]
    {
        ...
        let mut perm = file.metadata().map_err(Error::UnableToRetrieveMetadata)?.permissions();

        perm.set_mode(0o600);

        file.set_permissions(perm).map_err(|e| Error::UnableToSetPermissions(e))?;
    }

    file.write_all(bytes).map_err(|e| Error::UnableToWriteFile(e))?;
    #[cfg(windows)]
    {
...
pub fn restrict_file_permissions<P: AsRef<Path>>(path: P) -> Result<(), Error> {
    ...
    #[cfg(windows)]
    {
        ...
        let path_str = path.as_ref().to_str().ok_or(Error::UnableToObtainFilePath)?;
        let mut acl = ACL::from_file_path(&path_str, false).map_err(|e| Error::UnableToRetrieveACL(e))?;

        let owner_sid_str = "S-1-3-4";
        let owner_sid     = windows_acl::helper::string_to_sid(owner_sid_str).unwrap();

        let entries = acl.all().map_err(|e| Error::UnableToEnumerateACLEntries(e))?;

        // add single entry for file owner
        acl.add_entry(owner_sid.as_ptr() as PSID, AceType::AccessAllow, 0, 0x1f01ff).map_err(|code| Error::UnableToAddACLEntry(format!("Failed to add ACL entry for SID {} error={}", owner_sid_str, code)))?;
        ...
        for entry in &entries {
            if let Some(ref entry_sid) = entry.sid {
                ...
                if entry_sid_str != owner_sid_str {
                    acl.remove((*entry_sid).as_ptr() as PSID, Some(AceType::AccessAllow), None).map_err(|_| Error::UnableToRemoveACLEntry(format!("Failed to remove ACL entry for SID {}", entry_sid_str)))?;
                }
            }
        }
    }
