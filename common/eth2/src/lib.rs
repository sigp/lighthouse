...
impl BeaconNodeHttpClient {
    ...
    pub async fn get_events<E: EthSpec>(
        ...
        let response = self.client.get(path).timeout(Duration::MAX).send().await?;
        ...
        Ok(Box::pin(event_stream.filter_map(|event| async move {
            match event {
                Ok(sse_event) => Some(EventKind::from_sse_bytes(&sse_event.event, &sse_event.data)),
                Err(err) => Some(Err(Error::SseStream(err.to_string()))),
            }
        })))
    }
