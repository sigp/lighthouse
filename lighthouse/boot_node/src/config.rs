...
impl TryFrom<&ArgMatches<'_>> for BootNodeConfig {
    ...
    let enr_address = {
        let address_string = matches.value_of("boot-node-enr-address").expect("required parameter"); 
    ...
    let local_enr = discv5::enr::EnrBuilder::new("v4").ip(enr_address).udp(enr_port).build(&local_key).map_err(|e| format!("Failed to build ENR: {:?}", e))?;
...
fn resolve_address(address_string: String, port: u16) -> Result<IpAddr, String> {
        ...
        match address_string.parse::<IpAddr>() {
            ...
            Err(_) => {
                ...
                addr.to_socket_addrs().map(|mut resolved_addrs|  
                    // Pick the first ip from the list of resolved addresses
                    resolved_addrs
                        .next()
                        .map(|a| a.ip())
                        .ok_or_else(|| format!("Resolved dns addr contains no entries"))).map_err(|_| format!("Failed to parse enr-address: {}", address_string))?
            }
        }
}
