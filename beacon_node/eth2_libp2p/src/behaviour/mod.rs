        ...
        match event {
            ...
            GossipsubEvent::GossipsubNotSupported { peer_id } => {
                ...
                self.peer_manager.report_peer(
                    ...
                    Some(GoodbyeReason::ProtocolNotSupported),
                );
            }
        }
    }
...
impl<TSpec: EthSpec> NetworkBehaviourEventProcess<PeerManagerEvent> for Behaviour<TSpec> {
    fn inject_event(&mut self, event: PeerManagerEvent) {
        match event {
            PeerManagerEvent::PeerConnectedIncoming(peer_id) => self
                .events
                .push_back(BehaviourEvent::PeerConnectedIncoming(peer_id)),
            PeerManagerEvent::PeerConnectedOutgoing(peer_id) => self
                .events
                .push_back(BehaviourEvent::PeerConnectedOutgoing(peer_id)),
            PeerManagerEvent::PeerDisconnected(peer_id) => self
                .events
            ...
            PeerManagerEvent::Banned(peer_id, associated_ips) => {
                ...
                self.events.push_back(BehaviourEvent::PeerBanned(peer_id));
            }
            PeerManagerEvent::UnBanned(peer_id, associated_ips) => {
                self.discovery.unban_peer(&peer_id, associated_ips);
                self.events.push_back(BehaviourEvent::PeerUnbanned(peer_id));
            }
            PeerManagerEvent::Status(peer_id) => {
                ...
                self.events.push_back(BehaviourEvent::StatusPeer(peer_id));
            }
            ...
            PeerManagerEvent::RequestENRUpdate(peer_id) => {
                // Find and update the ENR of the peer_id if we know it
                if let Some(enr) = self.discovery.enr_of_peer(&peer_id) {
                    self.peer_manager.update_enr(&peer_id, enr);
                }
            }
        }
    }
