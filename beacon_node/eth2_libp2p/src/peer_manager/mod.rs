...
impl<TSpec: EthSpec> PeerManager<TSpec> {
            ...
            .filter(|s| {
                ...
                if peers_on_subnet >= TARGET_SUBNET_PEERS {
                    debug!(
                        ...
                        "subnet_id" => subnet_id_to_string(*s.subnet_id),
    ...
    pub fn identify(&mut self, peer_id: &PeerId, info: &IdentifyInfo) {
        if let Some(peer_info) = self.network_globals.peers.write().peer_info_mut(peer_id) {
            ...
            crit!(self.log, "Received an Identify response from an unknown peer"; "peer_id" => %peer_id);
        }
    }
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        loop {
            match self.inbound_ping_peers.poll_next_unpin(cx) {
                ...
                Poll::Ready(Some(Err(e))) => {
                    error!(self.log, "Failed to check for inbound peers to ping"; "error" => e)
                }
        ...
        loop {
            match self.outbound_ping_peers.poll_next_unpin(cx) {
                ...
                Poll::Ready(Some(Err(e))) => {
                    error!(self.log, "Failed to check for outbound peers to ping"; "error" => e)
                }
                Poll::Ready(None) | Poll::Pending => break,
            }
        }
        ...
        if !matches!(self.network_globals.sync_state(), SyncState::SyncingFinalized{..}|SyncState::SyncingHead{..})
        {
            loop {
                match self.status_peers.poll_next_unpin(cx) {
                    ...
                    Poll::Ready(Some(Err(e))) => {
                        error!(self.log, "Failed to check for peers to ping"; "error" => e)
                    }
                    Poll::Ready(None) | Poll::Pending => break,
                }
            }
