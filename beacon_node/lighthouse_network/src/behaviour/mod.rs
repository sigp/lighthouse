...
pub struct Network<AppReqId: ReqId, TSpec: EthSpec> {
    ...
    #[behaviour(ignore)]
    events: VecDeque<OldBehaviourEvent<AppReqId, TSpec>>,
...
impl<AppReqId: ReqId, TSpec: EthSpec> Network<AppReqId, TSpec> {
    ...
    fn propagate_response(
        ...
        match id {
            RequestId::Application(id) => self.add_event(OldBehaviourEvent::ResponseReceived {
                peer_id,
                id,
                response,
            }),
    ...
    fn propagate_request(&mut self, id: PeerRequestId, peer_id: PeerId, request: Request) {
        ...
        self.add_event(OldBehaviourEvent::RequestReceived {
            peer_id,
            id,
            request,
        });
    ...
    fn add_event(&mut self, event: OldBehaviourEvent<AppReqId, TSpec>) {
...
// Gossipsub
impl<AppReqId, TSpec> NetworkBehaviourEventProcess<GossipsubEvent> for Network<AppReqId, TSpec>
where
    AppReqId: ReqId,
    TSpec: EthSpec,
{
        ...
        match event {
            GossipsubEvent::Message {
                ...
                match PubsubMessage::decode(&gs_msg.topic, &gs_msg.data, &self.fork_context) {
                    ...
                    Ok(msg) => {
                        // Notify the network
                        self.add_event(OldBehaviourEvent::PubsubMessage {
                            id,
                ...
                match handler_err {
                    ...
                    HandlerErr::Outbound { id, proto, error } => {
                        ...
                        if let RequestId::Application(id) = id {
                            self.add_event(OldBehaviourEvent::RPCFailed { peer_id, id });
                        }
                    }
                }
            }
...
// Discovery
impl<AppReqId, TSpec> NetworkBehaviourEventProcess<DiscoveredPeers> for Network<AppReqId, TSpec>
where
    AppReqId: ReqId,
    TSpec: EthSpec,
{
...
// Identify
impl<AppReqId, TSpec> NetworkBehaviourEventProcess<IdentifyEvent> for Network<AppReqId, TSpec>
where
    TSpec: EthSpec,
    AppReqId: ReqId,
{
...
type BehaviourHandler<AppReqId, TSpec> =
    <Network<AppReqId, TSpec> as NetworkBehaviour>::ConnectionHandler;

impl<AppReqId, TSpec> Network<AppReqId, TSpec>
where
    TSpec: EthSpec,
    AppReqId: ReqId,
{
    ...
    ) -> Poll<NBAction<OldBehaviourEvent<AppReqId, TSpec>, BehaviourHandler<AppReqId, TSpec>>> {
        if let Some(waker) = &self.waker {
    ...
    fn inject_event(&mut self, event: PeerManagerEvent) {
        match event {
            PeerManagerEvent::PeerConnectedIncoming(peer_id) => {
                self.add_event(OldBehaviourEvent::PeerConnectedIncoming(peer_id));
            }
            PeerManagerEvent::PeerConnectedOutgoing(peer_id) => {
                self.add_event(OldBehaviourEvent::PeerConnectedOutgoing(peer_id));
            }
            PeerManagerEvent::PeerDisconnected(peer_id) => {
                self.add_event(OldBehaviourEvent::PeerDisconnected(peer_id));
            }
            PeerManagerEvent::Banned(peer_id, associated_ips) => {
                self.discovery.ban_peer(&peer_id, associated_ips);
                self.add_event(OldBehaviourEvent::PeerBanned(peer_id));
            }
            PeerManagerEvent::UnBanned(peer_id, associated_ips) => {
                self.discovery.unban_peer(&peer_id, associated_ips);
                self.add_event(OldBehaviourEvent::PeerUnbanned(peer_id));
            }
            PeerManagerEvent::Status(peer_id) => {
                ...
                self.add_event(OldBehaviourEvent::StatusPeer(peer_id));
            }
