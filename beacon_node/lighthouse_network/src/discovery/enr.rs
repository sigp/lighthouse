...
pub trait Eth2Enr {
    ...
    /// The execution proof subnets bitfield associated with the ENR.
    fn execution_proof_subnets(&self) -> Result<u8, &'static str>;
...
impl Eth2Enr for Enr {
    ...
    fn execution_proof_subnets(&self) -> Result<u8, &'static str> {
        self.get_decodable::<u8>(EXECUTION_PROOF_SUBNETS_ENR_KEY)
            .ok_or("ENR execution proof subnets non-existent")?
            .map_err(|_| "Could not decode the ENR execution proof subnets")
    }
...
fn compare_enr(local_enr: &Enr, disk_enr: &Enr) -> bool {
    ...
    (local_enr.ip6().is_none() || local_enr.ip6() == disk_enr.ip6())
        ...
        // PEERDAS_CUSTODY_GROUP_COUNT_ENR_KEY and EXECUTION_PROOF_SUBNETS_ENR_KEY keys to match,
        ...
        && local_enr.get_decodable::<u8>(EXECUTION_PROOF_SUBNETS_ENR_KEY) == disk_enr.get_decodable(EXECUTION_PROOF_SUBNETS_ENR_KEY)
}
