    ...
    async fn test_next_peer_request_ready() {
        ...
        for i in 1..=5u32 {
            let _ = limiter.allows(
                peer_id,
                RequestId::Application(AppRequestId::Sync(SyncRequestId::RangeBlockComponents(i))),
...

        {
            ...
            for i in 2..=5u32 {
                assert!(matches!(
                    ...
                    RequestId::Application(AppRequestId::Sync(SyncRequestId::RangeBlockComponents(id)
                    )) if id == i
                ));
            }
...

        {
            ...
            for i in 3..=5 {
                assert!(matches!(
                    iter.next().unwrap().request_id,
                    RequestId::Application(AppRequestId::Sync(SyncRequestId::RangeBlockComponents(id))) if id == i
                ));
            }
