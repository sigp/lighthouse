...
mod attestation_service {
    ...
    #[tokio::test]
    async fn test_subscribe_to_expiring_long_lived_subnet() {
        // subscription config
        let validator_index = 1;
        let committee_count = 1;
        ...
        // create the attestation service and subscriptions
        let attestation_service = get_attestation_service(Some(Level::Trace));
        let current_slot = attestation_service
            .beacon_chain
            .slot_clock
            .now()
        ...
        // There should be a long lived subnet now
        let long_lived_subnet = attestation_service
            .subscriptions(attestation_subnets::SubscriptionKind::LongLived)
            .keys()
            .next()
            .cloned()
            .expect("Should be subscribed to a long lived subnet");
        let long_lived_subnet_end_slot = attestation_service
            .subscriptions(attestation_subnets::SubscriptionKind::LongLived)
            .get(&long_lived_subnet)
            .cloned()
            .expect("Long lived subnet exists");

        // Craft a subscription that matches

        let sub2 = get_subscription(
            validator_index,
            long_lived_subnet_end_slot.as_u64(),
            long_lived_subnet_end_slot,
            0,
        );

        let subnet_id1 = SubnetId::compute_subnet::<MainnetEthSpec>(
            sub1.slot,
            sub1.attestation_committee_index,
            sub1.committee_count_at_slot,
            &attestation_service.beacon_chain.spec,
        )
        .unwrap();
        ...
        let subnet_id2 = SubnetId::compute_subnet::<MainnetEthSpec>(
            sub2.slot,
            sub2.attestation_committee_index,
            sub2.committee_count_at_slot,
            &attestation_service.beacon_chain.spec,
        )
        .unwrap();
