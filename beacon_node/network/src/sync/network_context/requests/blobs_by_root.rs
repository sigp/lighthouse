...
use std::sync::Arc;
use types::{BlobSidecar, ForkContext, Hash256, data::BlobIdentifier};
...
pub struct BlobsByRootRequestItems {
    request: BlobsByRootSingleBlockRequest,
    items: Vec<Arc<BlobSidecar>>,
}

impl BlobsByRootRequestItems {
...
impl ActiveRequestItems for BlobsByRootRequestItems {
    type Item = Arc<BlobSidecar>;
