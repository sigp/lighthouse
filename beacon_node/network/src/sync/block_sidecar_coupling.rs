...
mod tests {
    ...
    use tracing::Span;
    use types::{
        ChainSpec, DataColumnSidecarList, Epoch, ExecutionPayloadEnvelopeGloas, ForkName,
    ...
    fn matching_envelope(block: &SignedBeaconBlock<E>) -> Arc<SignedExecutionPayloadEnvelope<E>> {
        let bid = block
            ...
            .message();
        let mut message = ExecutionPayloadEnvelopeGloas::empty();
        message.beacon_block_root = block.canonical_root();
        message.parent_beacon_block_root = block.parent_root();
        message.builder_index = bid.builder_index();
        message.payload.slot_number = block.slot();
        message.payload.parent_hash = bid.parent_block_hash();
        message.payload.block_hash = bid.block_hash();
    ...
    fn gloas_payload_envelope_mismatch_fails_coupling() {
        ...
        match &mut bad_envelope {
            SignedExecutionPayloadEnvelope::Gloas(env) => env.message.payload.slot_number += 1,
            SignedExecutionPayloadEnvelope::Heze(env) => env.message.payload.slot_number += 1,
        }
