...
use beacon_chain::{
    ...
    test_utils::{
        ...
        generate_data_column_sidecars_from_block, generate_rand_block_and_blobs,
...
impl TestRig {
    ...
    fn parent_column_processed(&mut self, chain_hash: Hash256, result: BlockProcessingResult) {
        let id = self.find_single_lookup_for(self.find_oldest_parent_lookup(chain_hash));
        self.single_custody_column_component_processed(id, result);
    }
    ...
    fn single_custody_column_component_processed(&mut self, id: Id, result: BlockProcessingResult) {
        self.send_sync_message(SyncMessage::BlockComponentProcessed {
            process_type: BlockProcessType::SingleCustodyColumn(id),
            result,
        })
    }
    ...
    pub fn block_with_parent_and_data_columns(
        &mut self,
        parent_root: Hash256,
    ) -> (SignedBeaconBlock<E>, Vec<Arc<DataColumnSidecar<E>>>) {
        let (mut block, _) = self.rand_block_and_data_columns();
        *block.message_mut().parent_root_mut() = parent_root;
        let data_columns = generate_data_column_sidecars_from_block(&block, &self.harness.spec);
        (block, data_columns)
    }
    ...
    fn insert_column_to_da_checker(&mut self, column: Arc<DataColumnSidecar<E>>) {
        match self
            .harness
            .chain
            .data_availability_checker
            .put_gossip_verified_data_columns(
                column.block_root(),
                column.slot(),
                std::iter::once(GossipVerifiedDataColumn::<_, Observe>::__assumed_valid(
                    column,
                )),
            )
            .unwrap()
...
#[ignore]
#[test]
fn columns_in_da_checker_skip_download() {
    let Some(mut r) = TestRig::test_setup_after_deneb_before_fulu() else {
        return;
    };
    let (block, columns) = r.rand_block_and_data_columns();
    let block_root = block.canonical_root();
    let peer_id = r.new_connected_peer();
    for column in columns {
        r.insert_column_to_da_checker(column);
    }
    r.trigger_unknown_block_from_attestation(block_root, peer_id);
...
mod fulu_only {
    ...
    struct FuluTester {
        ...
        parent_data_columns: VecDeque<Vec<Arc<DataColumnSidecar<E>>>>,
        unknown_parent_block: Option<Arc<SignedBeaconBlock<E>>>,
        unknown_parent_data_columns: Option<Vec<Arc<DataColumnSidecar<E>>>>,
        peer_id: PeerId,
        block_req_id: Option<SingleLookupReqId>,
        parent_block_req_id: Option<SingleLookupReqId>,
        column_req_ids: Option<DCByRootIds>,
        parent_column_req_ids: Option<DCByRootIds>,
        slot: Slot,
        block_root: Hash256,
        sample_column_count: usize,
    }

    enum RequestTrigger {
        AttestationUnknownBlock,
        GossipUnknownParentBlock(usize),
        GossipUnknownParentDataColumn(usize),
    }
    ...
    impl RequestTrigger {
        fn num_parents(&self) -> usize {
            match self {
                ...
                RequestTrigger::GossipUnknownParentDataColumn(num_parents) => *num_parents,
            }
        }
    }
    ...
    impl FuluTester {
        fn new(request_trigger: RequestTrigger) -> Option<Self> {
            ...
            rig.new_connected_peers_for_peerdas();

            let spec = E::default_spec();
            let sample_column_count =
                (spec.samples_per_slot * spec.data_columns_per_group::<E>()) as usize;

            let (block, data_columns) = rig.rand_block_and_data_columns();
            let mut block = Arc::new(block);
            let mut data_columns = data_columns;
            let slot = block.slot();

            let num_parents = request_trigger.num_parents();
            let mut parent_block_chain = VecDeque::with_capacity(num_parents);
            let mut parent_data_columns_chain = VecDeque::with_capacity(num_parents);
            ...
            for _ in 0..num_parents {
                ...
                let parent_columns = data_columns.clone();
                parent_block_chain.push_front(parent_block);
                parent_data_columns_chain.push_front(parent_columns);
                parent_block_roots.push(parent_root);

                // Create the next block.
                let (child_block, child_data_columns) =
                    rig.block_with_parent_and_data_columns(parent_root);
                let mut child_block = Arc::new(child_block);
                let mut child_data_columns = child_data_columns;

                // Update the new block to the current block.
                std::mem::swap(&mut child_block, &mut block);
                std::mem::swap(&mut child_data_columns, &mut data_columns);
            }
            let block_root = block.canonical_root();
            ...
            // Trigger the request
            let (block_req_id, parent_block_req_id) = match request_trigger {
                ...
                RequestTrigger::GossipUnknownParentBlock { .. } => {
                    ...
                    (None, Some(parent_block_req_id))
                }
                RequestTrigger::GossipUnknownParentDataColumn { .. } => {
                    let single_column = data_columns.first().cloned().unwrap();
                    let parent_root = single_column.as_fulu().unwrap().block_parent_root();
                    rig.trigger_unknown_parent_data_column(peer_id, single_column);
            ...
            Some(Self {
                ...
                parent_data_columns: parent_data_columns_chain,
                parent_block_roots,
                unknown_parent_block: None,
                unknown_parent_data_columns: None,
                peer_id,
                block_req_id,
                parent_block_req_id,
                column_req_ids: None,
                parent_column_req_ids: None,
                slot,
                block_root,
                sample_column_count,
            })
        ...
        fn parent_block_response_expect_columns(mut self) -> Self {
            self.rig.expect_empty_network();
            let block = self.parent_block.pop_front().unwrap().clone();
            let block_root = block.canonical_root();
            let _ = self.unknown_parent_block.insert(block.clone());
            ...
            // Expect data columns request after sending block
            let s = self.expect_parent_columns_request(block_root);

            s.rig.assert_parent_lookups_count(1);
            s
        }

        fn parent_column_response(mut self) -> Self {
            let data_columns = self.parent_data_columns.pop_front().unwrap();
            let _ = self
                .unknown_parent_data_columns
                .insert(data_columns.clone());

            let column_req_ids = self
                .parent_column_req_ids
                .take()
                .expect("parent column request ids");

            // Send responses for each custody request
            for id in column_req_ids {
                let indices = &id.1;
                let columns_to_send: Vec<_> = indices
                    ...
                    .collect();
                self.rig
                    .complete_data_columns_by_root_request(id, &columns_to_send);
            }
        ...
        fn block_response_triggering_process(self) -> Self {
            let mut me = self.block_response_and_expect_column_request();
        ...
        fn block_response_and_expect_column_request(mut self) -> Self {
            // The peer provides the correct block, should not be penalized.
            self.rig.single_lookup_block_response(
                self.block_req_id.expect("block request id"),
                self.peer_id,
                Some(self.block.clone()),
            );
            // After responding with block the node will issue data column requests
        ...
        fn data_columns_response(mut self) -> Self {
            self.rig.log(&format!(
                "data columns response {}",
                self.data_columns.len()
            ));

            let column_req_ids = self.column_req_ids.take().expect("column request ids");
            ...
            for id in column_req_ids {
                ...
                let columns_to_send: Vec<_> = indices
                    .iter()
                    .map(|&i| self.data_columns[i as usize].clone())
                    .collect();
                self.rig
                    .complete_data_columns_by_root_request(id, &columns_to_send);
            }
        ...
        fn data_columns_response_was_valid(mut self) -> Self {
            self.rig.expect_empty_network();
            if !self.data_columns.is_empty() {
                self.rig.expect_block_process(ResponseType::CustodyColumn);
            }
            self
        }
        ...
        fn empty_data_columns_response(mut self) -> Self {
            let column_req_ids = self.column_req_ids.take().expect("column request ids");

            // Send empty responses for each custody request
            for (sync_request_id, _) in column_req_ids {
                // Send stream termination with no data
                self.rig.send_sync_message(SyncMessage::RpcDataColumn {
                    sync_request_id,
        ...
        fn empty_parent_columns_response(mut self) -> Self {
            let column_req_ids = self
                .parent_column_req_ids
                .take()
                .expect("parent column request ids");
        ...
        fn custody_columns_imported(mut self) -> Self {
            let lookup_id = self.get_custody_lookup_id();
            self.rig
                .send_sync_message(SyncMessage::BlockComponentProcessed {
                    process_type: BlockProcessType::SingleCustodyColumn(lookup_id),
        ...
        fn get_custody_lookup_id(&self) -> Id {
            if let Some(ref ids) = self.column_req_ids
                && let SyncRequestId::DataColumnsByRoot(DataColumnsByRootRequestId {
                    requester: DataColumnsByRootRequester::Custody(id),
                    ..
                }) = ids.first().unwrap().0
            {
                return id.requester.0.lookup_id;
            }

            if let Some(id) = self.block_req_id {
                return id.lookup_id;
            }
            panic!("No custody lookup id found")
        }
        ...
        fn parent_columns_imported(mut self) -> Self {
            let parent_root = *self.parent_block_roots.first().unwrap();
            self.rig
                .log(&format!("parent_columns_imported {parent_root:?}"));

            self.rig.parent_column_processed(
                self.block_root,
                BlockProcessingResult::Ok(AvailabilityProcessingStatus::Imported(parent_root)),
            );
        ...
        fn invalid_column_processed(mut self) -> Self {
            self.rig.log("invalid_column_processed");
            let lookup_id = self.get_custody_lookup_id();
            self.rig
                .send_sync_message(SyncMessage::BlockComponentProcessed {
                    process_type: BlockProcessType::SingleCustodyColumn(lookup_id),
                    result: BlockProcessingResult::Err(BlockError::AvailabilityCheck(
                        AvailabilityCheckError::InvalidColumn((
                            None,
                            kzg::Error::KzgVerificationFailed,
                        )),
                    )),
                });
            self.rig.assert_single_lookups_count(1);
        ...
        fn missing_components_from_block_request(mut self) -> Self {
            ...
            // Add block to da_checker so columns request can continue
        ...
        fn complete_current_block_and_columns_lookup(self) -> Self {
            self.expect_block_request()
                .block_response_and_expect_column_request()
        ...
        fn parent_block_then_empty_parent_columns(self) -> Self {
            self.log(
                " Return empty columns for parent, block errors with missing components, downscore",
            )
            .parent_block_response()
            .expect_parent_columns_request_for_unknown_parent()
            .empty_parent_columns_response()
            .expect_penalty("NotEnoughResponsesReturned")
            .log("Re-request parent columns, succeed and import parent")
            .expect_parent_columns_request_for_unknown_parent()
            .parent_column_response()
            .expect_block_process()
        ...
        fn expect_columns_request(mut self) -> Self {
            let ids = self.rig.expect_data_columns_by_root_requests(
                self.block.canonical_root(),
                self.sample_column_count,
            );
            self.column_req_ids = Some(ids);
            self
        }
        ...
        fn expect_parent_columns_request(mut self, block_root: Hash256) -> Self {
            let ids = self
                .rig
                .expect_data_columns_by_root_requests(block_root, self.sample_column_count);
            self.parent_column_req_ids = Some(ids);
            self
        }
        fn expect_parent_columns_request_for_unknown_parent(mut self) -> Self {
            let block_root = self
        ...
        fn expect_no_columns_request(mut self) -> Self {
            self.rig.expect_empty_network();
            self
        }
        ...
        fn invalidate_columns_too_few(mut self) -> Self {
            self.data_columns.pop().expect("data_columns");
            self
        }
    ...
    #[test]
    fn single_block_and_column_lookup_block_returned_first_attestation() {
        let Some(tester) = FuluTester::new(RequestTrigger::AttestationUnknownBlock) else {
            return;
        };
        tester
            .block_response_and_expect_column_request()
            .data_columns_response()
            .block_missing_components() // columns not yet imported
            .data_columns_response_was_valid()
            .custody_columns_imported(); // now columns resolve as imported
    }

    #[test]
    fn single_block_response_then_empty_column_response_attestation() {
        let Some(tester) = FuluTester::new(RequestTrigger::AttestationUnknownBlock) else {
            return;
        };
        tester
            ...
            .empty_data_columns_response()
            .expect_penalty("NotEnoughResponsesReturned")
            .expect_columns_request()
            .expect_no_block_request();
    }

    #[test]
    fn single_invalid_block_response_then_column_response_attestation() {
        let Some(tester) = FuluTester::new(RequestTrigger::AttestationUnknownBlock) else {
            return;
        };
        tester
            ...
            .expect_no_columns_request()
            .data_columns_response()
            // columns not sent for processing until the block is processed
            .expect_no_penalty_and_no_requests();
    }

    #[test]
    fn single_block_response_then_invalid_column_response_attestation() {
        let Some(tester) = FuluTester::new(RequestTrigger::AttestationUnknownBlock) else {
            return;
        };
        tester
            ...
            .missing_components_from_block_request()
            .data_columns_response()
            .invalid_column_processed()
            .expect_penalty("lookup_custody_column_processing_failure")
            .expect_columns_request()
            .expect_no_block_request();
    }

    #[test]
    fn single_block_response_then_too_few_columns_response_attestation() {
        let Some(tester) = FuluTester::new(RequestTrigger::AttestationUnknownBlock) else {
            return;
        };
        tester
    ...
    fn parent_block_unknown_parent() {
        let Some(tester) = FuluTester::new(RequestTrigger::GossipUnknownParentBlock(1)) else {
            return;
        };
        tester
            .expect_empty_beacon_processor()
            .parent_block_response_expect_columns()
            .parent_column_response()
            .expect_block_process()
    ...
    #[test]
    fn parent_block_then_empty_parent_columns() {
        let Some(tester) = FuluTester::new(RequestTrigger::GossipUnknownParentBlock(1)) else {
            return;
        };
        tester
            .parent_block_then_empty_parent_columns()
            .log("resolve original block trigger columns request and import")
            // Should not have block request, it is cached
            .expect_columns_request()
            // TODO: Should send columns for processing
            .block_imported()
            .expect_no_active_lookups();
    }

    #[test]
    fn parent_column_unknown_parent() {
        let Some(tester) = FuluTester::new(RequestTrigger::GossipUnknownParentDataColumn(1)) else {
            return;
        };
        tester
            .expect_empty_beacon_processor()
    ...
    #[test]
    fn parent_column_invalid_parent() {
        let Some(tester) = FuluTester::new(RequestTrigger::GossipUnknownParentDataColumn(1)) else {
            return;
        };
        tester
            ...
            .expect_penalty("lookup_block_processing_failure")
            .expect_parent_block_request()
            // columns are not sent until block is processed
            .expect_empty_beacon_processor();
    }

    #[test]
    fn parent_block_and_column_lookup_parent_returned_first_column_trigger() {
        let Some(tester) = FuluTester::new(RequestTrigger::GossipUnknownParentDataColumn(1)) else {
            return;
        };
        tester
            ...
            .complete_current_block_and_columns_lookup()
            .expect_no_active_lookups();
    }

    #[test]
    fn parent_block_then_empty_parent_columns_column_trigger() {
        let Some(tester) = FuluTester::new(RequestTrigger::GossipUnknownParentDataColumn(1)) else {
            return;
        };
        tester
    ...
    #[test]
    fn parent_column_unknown_parent_chain() {
        let Some(tester) = FuluTester::new(RequestTrigger::GossipUnknownParentDataColumn(2)) else {
            return;
        };
        tester
            .expect_empty_beacon_processor()
