...
impl<T: BeaconChainTypes> NetworkBeaconProcessor<T> {
    ...
    pub async fn publish_partial_data_columns(
        ...
        let messages = crate::build_partial_column_request_messages(
            self.chain.as_ref(),
            &header_or_bid,
            block_root,
        );
        ...
        } else if !header_or_bid.kzg_commitments().is_empty()
            && self.chain.config.enable_partial_columns
        {
            // This should not happen, as any custody columns will have at least an empty
            // partial published when partial columns are enabled and commitments exist.
