...
impl<T: BeaconChainTypes> NetworkBeaconProcessor<T> {
    ...
    /// Create a new work event to process `InclusionListsByCommitteeIndicesRequest`s from the
    /// RPC network.
    pub fn send_inclusion_lists_by_committee_indices_request(
        self: &Arc<Self>,
        peer_id: PeerId,
        inbound_request_id: InboundRequestId,
        ...
        let processor = self.clone();
        let process_fn = move || {
            processor.handle_inclusion_lists_by_committee_indices_request(
                peer_id,
                inbound_request_id,
                request,
            )
        ...
        self.try_send(BeaconWorkEvent {
            drop_during_sync: false,
            work: Work::InclusionListsByCommitteeIndicesRequest(Box::new(process_fn)),
        })
    }
