...
#[behaviour(out_event = "BehaviourEvent<TChain::EthSpec>", poll_method = "poll")]
pub struct Behaviour<TSubstream: AsyncRead + AsyncWrite, TChain: BeaconChainTypes> {
    /// The routing pub-sub mechanism for eth2.
    gossipsub: Gossipsub<TSubstream>,
    /// The Eth2 RPC specified in the wire-0 protocol.
    eth2_rpc: RPC<TSubstream, TChain::EthSpec>,
    /// Keep regular connection to peers and disconnect if absent.
    // TODO: Using id for initial interop. This will be removed by mainnet.
    /// Provides IP addresses and peer information.
    identify: Identify<TSubstream>,
    /// Discovery behaviour.
    discovery: Discovery<TSubstream, TChain::EthSpec>,
    /// The events generated by this behaviour to be consumed in the swarm poll.
    #[behaviour(ignore)]
    events: Vec<BehaviourEvent<TChain::EthSpec>>,
    ...
    #[behaviour(ignore)]
    network_globals: Arc<NetworkGlobals<TChain::EthSpec>>,
    #[behaviour(ignore)]
    /// A reference to the underlying beacon chain to keep track of fork versions via the slot
    /// clock.
    #[behaviour(ignore)]
    beacon_chain: Arc<BeaconChain<TChain>>,
    #[behaviour(ignore)]
    /// Logger for behaviour actions.
    log: slog::Logger,
}

impl<TSubstream: AsyncRead + AsyncWrite, TChain: BeaconChainTypes> Behaviour<TSubstream, TChain> {
    pub fn new(
        local_key: &Keypair,
        ...
        let enr_fork_id = beacon_chain
            .enr_fork_id()
            .map_err(|e| format!("Beacon chain couldn't get the ENR fork id: {:?}", e))?;
    ...
    pub fn discovery(&self) -> &Discovery<TSubstream, TChain::EthSpec> {
        &self.discovery
    }
...
impl<TSubstream: AsyncRead + AsyncWrite, TChain: BeaconChainTypes>
    NetworkBehaviourEventProcess<GossipsubEvent> for Behaviour<TSubstream, TChain>
{
...
impl<TSubstream: AsyncRead + AsyncWrite, TChain: BeaconChainTypes>
    NetworkBehaviourEventProcess<RPCMessage<TChain::EthSpec>> for Behaviour<TSubstream, TChain>
{
    fn inject_event(&mut self, event: RPCMessage<TChain::EthSpec>) {
        match event {
...
impl<TSubstream: AsyncRead + AsyncWrite, TChain: BeaconChainTypes> Behaviour<TSubstream, TChain> {
    ...
    fn poll<TBehaviourIn>(
        &mut self,
    ) -> Async<NetworkBehaviourAction<TBehaviourIn, BehaviourEvent<TChain::EthSpec>>> {
...
impl<TSubstream: AsyncRead + AsyncWrite, TChain: BeaconChainTypes>
    NetworkBehaviourEventProcess<IdentifyEvent> for Behaviour<TSubstream, TChain>
{
...
impl<TSubstream: AsyncRead + AsyncWrite, TChain: BeaconChainTypes>
    NetworkBehaviourEventProcess<Discv5Event> for Behaviour<TSubstream, TChain>
{
...
impl<TSubstream: AsyncRead + AsyncWrite, TChain: BeaconChainTypes> Behaviour<TSubstream, TChain> {
    ...
    pub fn publish(&mut self, messages: Vec<PubsubMessage<TChain::EthSpec>>) {
    ...
    pub fn send_rpc(&mut self, peer_id: PeerId, rpc_event: RPCEvent<TChain::EthSpec>) {
