...
use types::{
    ...
    PartialDataColumnSidecarRef, SignedExecutionPayloadBid,
};
...
impl<T: BeaconChainTypes> PendingPayloadCache<T> {
    ...
    pub fn get_bid(
        ...
        self.peek_pending_components(block_root, move |components| {
            components.map(move |components| components.bid.clone())
        })
    }
...
mod data_availability_checker_tests {
    ...
    use types::{
        ExecutionPayloadEnvelopeGloas, ExecutionPayloadGloas, ExecutionRequests, ForkName,
        MinimalEthSpec, SignedExecutionPayloadEnvelope, SignedExecutionPayloadEnvelopeGloas,
    };
    ...
    fn executed_envelope(block_root: Hash256) -> AvailabilityPendingExecutedEnvelope<E> {
        AvailabilityPendingExecutedEnvelope {
            envelope: Arc::new(SignedExecutionPayloadEnvelope::Gloas(
