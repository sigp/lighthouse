...
impl<T: EthSpec> ValidatorMonitor<T> {
    ...
    pub fn get_unaggregated_attestation(&mut self, slot: Slot) -> Option<&Attestation<T>> {
    ...
    pub fn process_valid_state(
        ...
        //check unaggregated attestation
    ...
    /// Process the unaggregated attestations generated by the 'timer' service
    fn process_unaggregated_attestations(&mut self, state: &BeaconState<T>, spec: &ChainSpec) {
        ...
        if let Some(unaggregated_attestation) = self.get_unaggregated_attestation(slot) {
            let inclusion_delay = state.slot() - unaggregated_attestation.data.slot;
            let _ = get_attestation_participation_flag_indices(
                state,
                &unaggregated_attestation.data,
                inclusion_delay.as_u64(),
                spec,
            );
        }
    }
