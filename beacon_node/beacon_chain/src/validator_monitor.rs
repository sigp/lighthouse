...
use types::{
    ...
    BeaconStateError, ChainSpec, Epoch, Hash256, IndexedAttestation, IndexedAttestationRef,
    ProposerSlashing, SignedAggregateAndProof, SignedContributionAndProof, Slot, Spec,
...
pub struct ValidatorMonitor {
    ...
    unaggregated_attestations: HashMap<Slot, Attestation>,
}
...
impl ValidatorMonitor {
    ...
    pub fn set_unaggregated_attestation(&mut self, attestation: Attestation) {
    ...
    pub fn get_unaggregated_attestation(&self, slot: Slot) -> Option<&Attestation> {
    ...
    /// Add missed non-finalized blocks for the monitored validators
    fn add_validators_missed_blocks(&mut self, state: &BeaconState, spec: &ChainSpec) {
    ...
    fn get_proposers_by_epoch_from_cache(
        ...
        cache.get_epoch(shuffling_decision_block, epoch).cloned()
    }
    ...
    fn process_unaggregated_attestations(&mut self, state: &BeaconState, spec: &ChainSpec) {
        let current_slot = state.slot();
    ...
    pub fn register_attestation_in_block(
        &self,
        indexed_attestation: IndexedAttestationRef<'_>,
        parent_slot: Slot,
        spec: &ChainSpec,
    ) {
        let data = indexed_attestation.data();
    ...
    pub fn register_gossip_attester_slashing(&self, slashing: AttesterSlashingRef<'_>) {
        self.register_attester_slashing("gossip", slashing)
    }

    /// Register an attester slashing from the HTTP API.
    pub fn register_api_attester_slashing(&self, slashing: AttesterSlashingRef<'_>) {
        self.register_attester_slashing("api", slashing)
    }

    /// Register an attester slashing included in a *valid* `BeaconBlock`.
    pub fn register_block_attester_slashing(&self, slashing: AttesterSlashingRef<'_>) {
        self.register_attester_slashing("block", slashing)
    }

    fn register_attester_slashing(&self, src: &str, slashing: AttesterSlashingRef<'_>) {
...
pub fn get_block_delay_ms<S: SlotClock, P: AbstractExecPayload>(
    seen_timestamp: Duration,
    block: BeaconBlockRef<'_, P>,
    slot_clock: &S,
) -> Duration {
