...
pub struct MonitoredValidator<T: EthSpec> {
    ...
    pub unaggregated_attestations: HashMap<Slot, Attestation<T>>,
}
...
pub struct ValidatorMonitor<T> {
    ...
    validators: HashMap<PublicKeyBytes, MonitoredValidator<T>>,
...
impl<T: EthSpec> ValidatorMonitor<T> {
    ...
    fn process_attestations(&mut self, state: &BeaconState<T>) {
        // Define range variables
        let current_slot = state.slot();

        let attestation_slot = current_slot - Slot::new(UNAGGREGATED_ATTESTATION_LAG_SLOTS as u64);

        state.get_attestation_duties_at_slot()
    }
    ...
    fn get_validator(&self, validator_index: u64) -> Option<&MonitoredValidator<T>> {
        self.indices
            .get(&validator_index)
