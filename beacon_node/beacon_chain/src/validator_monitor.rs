...
impl<T: EthSpec> ValidatorMonitor<T> {
    ...
    /// Process the unaggregated attestations generated by the service `attester_per_slot_service`
    /// and check if the attestation qualifies for a reward matching the flags source/atrget/head
    fn process_unaggregated_attestations(&mut self, state: &BeaconState<T>, spec: &ChainSpec) {
        ...
        // If there is an unaggregated attestation for the slot
