...
pub fn backfill_light_client_epoch_data<T: BeaconChainTypes>(
    ...
    // Resume from the watermark, not from a per-period existence check.
    // The "watermark" here refers to the highest sync committee period 
...
/// the period is fed through the existing update path; it already retains
/// only the spec-best block per period via `is_better_light_client_update`,
/// so no separate candidate-selection step is needed here.
fn backfill_period<T: BeaconChainTypes>(
    ...
    for result in chain.forwards_iter_block_roots_until(start_slot, end_slot)? {
        let (block_root, _slot) = result?;

        let Some(block) = chain.get_blinded_block(&block_root)? else {
            continue; // skipped/empty slot
        };
