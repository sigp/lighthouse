...
impl<T: BeaconChainTypes> BeaconChain<T> {
    ...
    pub fn get_expected_withdrawals(
        ...
        let parent_payload_status = if let Some(block_hash) = parent_bid_block_hash
            ...
            && forkchoice_update_params.head_hash == FcuHash::Hash(block_hash)
        {
    ...
    pub async fn update_execution_engine_forkchoice(
        ...
        let (
            FcuHash::Hash(head_hash),
            FcuHash::Hash(justified_hash),
            FcuHash::Hash(finalized_hash),
        ) = (
            params.head_hash,
            params.justified_hash,
            params.finalized_hash,
        )
        else {
            // At least one of head / justified / finalized is pre-merge. We refuse to send any
            // zero hash to the EL, so we skip the fcU and wait for the merge block to be
            // finalized — that happens at most ~2 epochs after the merge, so delaying the first
            // fcU on a fresh genesis sync by 2 epochs is harmless.
            //
            // Past the Capella fork this state is impossible (the merge finalized many epochs
            // earlier), so reaching here indicates a corrupted fork choice.
            ...
            if self
                ...
                .capella_enabled()
            {
                return Err(Error::PreMergeBlockAtCapella {
