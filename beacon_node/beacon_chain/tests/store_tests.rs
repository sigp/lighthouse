...
use beacon_chain::{
    ...
    historical_blocks::HistoricalDataColumnError, migrate::MigratorConfig, BeaconChain,
...
async fn test_import_historical_data_columns_batch() {
    ...
    for block in block_root_iter {
        ...
        data_columns_list.push(data_columns.unwrap());
    }
    ...
    harness
        .chain
        .import_historical_data_column_batch(data_columns_list)
        .unwrap();
...
async fn test_import_historical_data_columns_batch_mismatched_block_root() {
    ...
    assert!(matches!(
        error,
        HistoricalDataColumnError::MismatchedBlockRoot { .. }
    ));
}
...
#[tokio::test]
async fn test_import_historical_data_columns_batch_invalid_signature() {
    let spec = ForkName::Fulu.make_genesis_spec(E::default_spec());
    let db_path = tempdir().unwrap();
    let store = get_store_generic(&db_path, StoreConfig::default(), spec);
    let start_slot = Slot::new(1);
