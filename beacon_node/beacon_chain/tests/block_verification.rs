...
fn update_envelope_block_root(snapshot: &mut BeaconSnapshot<E>) {
    if let Some(envelope) = snapshot.execution_envelope.as_ref() {
        ...
        let beacon_block_root = snapshot.beacon_block.canonical_root();
        match &mut envelope {
            SignedExecutionPayloadEnvelope::Gloas(env) => {
                env.message.beacon_block_root = beacon_block_root
            }
            SignedExecutionPayloadEnvelope::Heze(env) => {
                env.message.beacon_block_root = beacon_block_root
            }
        }
...
async fn range_sync_block_new_gloas_rejects_builder_index_mismatch() {
    ...
    match &mut envelope {
        SignedExecutionPayloadEnvelope::Gloas(env) => env.message.builder_index += 1,
        SignedExecutionPayloadEnvelope::Heze(env) => env.message.builder_index += 1,
    }
    let available_envelope = AvailableEnvelope::new(Arc::new(envelope), vec![]);
    let result = RangeSyncBlock::new_gloas(block, Some(available_envelope));
...
async fn range_sync_block_new_gloas_rejects_block_hash_mismatch() {
    ...
    match &mut envelope {
        SignedExecutionPayloadEnvelope::Gloas(env) => {
            env.message.payload.block_hash = ExecutionBlockHash::repeat_byte(0x22)
        }
        SignedExecutionPayloadEnvelope::Heze(env) => {
            env.message.payload.block_hash = ExecutionBlockHash::repeat_byte(0x22)
        }
