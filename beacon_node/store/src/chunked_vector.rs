//! DEPRECATED. Can be deleted once all nodes are on schema v24.
//!
...
field!(
    ...
    |state: &BeaconState<_>, index, _| safe_modulo_index_vector(state.block_roots(), index)
);
...
field!(
    ...
    |state: &BeaconState<_>, index, _| safe_modulo_index_vector(state.state_roots(), index)
);
...
field!(
    ...
    |state: &BeaconState<_>, index, _| safe_modulo_index_vector(state.randao_mixes(), index)
);
...
fn safe_modulo_index_list<T: Copy + milhouse::Value, N: Unsigned>(
    values: &List<T, N>,
    index: u64,
) -> Result<T, ChunkError> {
    if values.is_empty() {
