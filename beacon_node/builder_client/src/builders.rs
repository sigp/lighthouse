...
use eth2::types::{
    BuilderEntry, BuilderPreferenceEntry, BuilderPreferences, BuilderPreferencesRequest, EthSpec,
    ExecutionBlockHash, Hash256, SignedBeaconBlock, SignedExecutionPayloadBid, Slot,
};
...
impl Builders {
    ...
    /// entry's expected `builder_pubkey` (`None` when the entry omits one) is passed to `validate`
