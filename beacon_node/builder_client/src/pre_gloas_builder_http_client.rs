...
#[derive(Clone)]
pub struct PreGloasBuilderHttpClient {
...
impl PreGloasBuilderHttpClient {
    pub fn new(
        server: SensitiveUrl,
    ...
    >(
        ...
        let Ok(Some(fork_name)) = fork_name_from_header(&headers) else {
        ...
        let content_type = content_type_from_header(&headers);
