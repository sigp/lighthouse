...
use eth2::types::{
    BuilderPreferencesRequest, ContentType, EthSpec, ExecutionBlockHash, ForkVersionedResponse,
...
impl BuilderHttpClient {
    ...
    /// response via the `Accept` header, and the response is decoded according to its
    /// `Content-Type`.
    ...
    ///
    /// `BuilderPreferencesRequest` is not fork-versioned, so no `Eth-Consensus-Version` header is
