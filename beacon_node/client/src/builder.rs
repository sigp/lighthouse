    ...
    TColdStore: ItemStore<E> + 'static,
{
    ...
    pub async fn beacon_chain_builder(
        ...
        let context = runtime_context.service_context();
    ...
    pub fn build(
        ...
        if let Some(beacon_chain) = self.beacon_chain.as_ref() {
            if let Some(network_globals) = &self.network_globals {
                let beacon_processor_context = runtime_context.service_context();
                BeaconProcessor {
            ...
            let state_advance_context = runtime_context.service_context();
