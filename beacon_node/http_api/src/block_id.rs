...
impl BlockId {
    ...
    pub fn root<T: BeaconChainTypes>(
        ...
        match &self.0 {
            CoreBlockId::Head => Ok(chain.chain_summary().head_block_root),
            CoreBlockId::Genesis => Ok(chain.genesis_block_root),
            CoreBlockId::Finalized => Ok(chain.chain_summary().finalized_checkpoint.root),
            CoreBlockId::Justified => Ok(chain.chain_summary().justified_checkpoint.root),
