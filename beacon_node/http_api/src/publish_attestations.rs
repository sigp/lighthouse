...
fn verify_and_publish_single_attestation<T: BeaconChainTypes>(
    ...
    let attestation = chain
        .verify_single_attestation_for_gossip(attestation, None)
...
fn verify_and_publish_attestation<T: BeaconChainTypes>(
    ...
    match attestation.attestation() {
        ...
        types::AttestationRef::Electra(_) => {
            // TODO(single-attestation) this should be a no-op or error
            todo!()
        }
    };
    ...
    let mut prelim_results = task_spawner
        .blocking_task(Priority::P0, move || {
            Ok(attestations
                .into_iter()
                .map(|attestation| {
                    match verify_and_publish_single_attestation(
                        &chain,
                        &attestation,
                        seen_timestamp,
                        &network_tx,
                        ...
                        Err(Error::Validation(AttestationError::UnknownHeadBlock {
                            ...
                            let reprocess_fn = move || {
                                let result = verify_and_publish_single_attestation(
                                    &reprocess_chain,
                                    &attestation,
                                    seen_timestamp,
