use types::{BeaconState, AttesterSlashing, ChainSpec, EthSpec};
...
impl <'a, T: EthSpec> AttesterSlashingMaxCover<'a, T> {
    pub fn new(
        ...
        let length = slashing.attestation_1.attesting_indices.len() + slashing.attestation_2.attesting_indices.len();
...
impl <'a, T: EthSpec> MaxCover for AttesterSlashingMaxCover<'a, T> {
    ...
    fn update_covering_set(&mut self, best_slashing: &AttesterSlashing<T>, covered_validator_indices: &HashMap<u64, u64>) {
        if best_slashing == self.slashing {
            return;
        }

        self.effective_balances.retain(
            |k, _| !covered_validator_indices.contains_key(k)
        );
    }

    /// The quality of this item's covering set, usually its cardinality.
    fn score(&self) -> usize {
        self.effective_balances.values().sum::<u64>() as usize + self.effective_balances.keys().len()
    }
}