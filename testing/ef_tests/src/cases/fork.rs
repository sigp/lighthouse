...
use state_processing::upgrade::{
    ...
    upgrade_to_deneb, upgrade_to_electra, upgrade_to_fulu, upgrade_to_gloas,
};
...
impl<E: EthSpec> Case for ForkTest<E> {
    ...
    fn result(&self, _case_index: usize, fork_name: ForkName) -> Result<(), Error> {
        ...
        let mut result = match fork_name {
            ...
            ForkName::Gloas => {
                upgrade_to_gloas(&mut result_state, GloasVerificationContext::FullVerification, spec).map(|_| result_state)
            }
        };
