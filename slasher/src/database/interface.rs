...
#[cfg(feature = "rocksdb")]
use crate::database::rocksdb_impl;
...
pub enum Environment {
    ...
    #[cfg(feature = "rocksdb")]
    RocksDb(rocksdb_impl::Environment),
    Disabled,
}
...
pub enum RwTransaction<'env> {
    ...
    #[cfg(feature = "rocksdb")]
    RocksDb(rocksdb_impl::RwTransaction<'env>),
...
pub enum Database<'env> {
    ...
    #[cfg(feature = "rocksdb")]
    RocksDb(rocksdb_impl::Database<'env>),
    Disabled(PhantomData<&'env ()>),
}
...
pub enum Cursor<'env> {
    ...
    #[cfg(feature = "rocksdb")]
    RocksDb(rocksdb_impl::Cursor<'env>),
    Disabled(PhantomData<&'env ()>),
}
...
impl Environment {
    pub fn new(config: &Config) -> Result<Environment, Error> {
        match config.backend {
            ...
            #[cfg(feature = "rocksdb")]
            DatabaseBackend::RockDb => rocksdb_impl::Environment::new(config).map(Environment::RocksDb),
    ...
    pub fn create_databases(&self) -> Result<OpenDatabases, Error> {
        match self {
            ...
            #[cfg(feature = "rocksdb")]
            Self::RocksDb(env) => env.create_databases(),
    ...
    pub fn begin_rw_txn(&self) -> Result<RwTransaction, Error> {
        match self {
            ...
            #[cfg(feature = "rocksdb")]
            Self::RocksDb(env) => env.begin_rw_txn().map(RwTransaction::RocksDb),
            _ => Err(Error::MismatchedDatabaseVariant),
        }
    }
    ...
    pub fn filenames(&self, config: &Config) -> Vec<PathBuf> {
        match self {
            ...
            #[cfg(feature = "rocksdb")]
            Self::RocksDb(env) => env.filenames(config),
            _ => vec![],
        }
    }
}
...
impl<'env> RwTransaction<'env> {
    pub fn get<K: AsRef<[u8]> + ?Sized>(
        ...
        match (self, db) {
            ...
            #[cfg(feature = "rocksdb")]
            (Self::RocksDb(txn), Database::RocksDb(db)) => txn.get(db, key),
            _ => Err(Error::MismatchedDatabaseVariant),
        }
    }
    ...
    pub fn put<K: AsRef<[u8]>, V: AsRef<[u8]>>(
        ...
        match (self, db) {
            ...
            #[cfg(feature = "rocksdb")]
            (Self::RocksDb(txn), Database::RocksDb(db)) => txn.put(db, key, value),
            _ => Err(Error::MismatchedDatabaseVariant),
        }
    }
    ...
    pub fn del<K: AsRef<[u8]>>(&mut self, db: &Database, key: K) -> Result<(), Error> {
        match (self, db) {
            ...
            #[cfg(feature = "rocksdb")]
            (Self::RocksDb(txn), Database::RocksDb(db)) => txn.del(db, key),
            _ => Err(Error::MismatchedDatabaseVariant),
        }
    }
    ...
    pub fn cursor<'a>(&'a mut self, db: &Database) -> Result<Cursor<'a>, Error> {
        match (self, db) {
            ...
            #[cfg(feature = "rocksdb")]
            (Self::RocksDb(txn), Database::RocksDb(db)) => txn.cursor(db).map(Cursor::RocksDb),
            _ => Err(Error::MismatchedDatabaseVariant),
        }
    }
...
impl<'env> Cursor<'env> {
    ...
    pub fn first_key(&mut self) -> Result<Option<Key>, Error> {
        match self {
            ...
            #[cfg(feature = "rocksdb")]
            Cursor::RocksDb(cursor) => cursor.first_key(),
            _ => Err(Error::MismatchedDatabaseVariant),
        }
    }
    ...
    pub fn last_key(&mut self) -> Result<Option<Key>, Error> {
        match self {
            ...
            #[cfg(feature = "rocksdb")]
            Cursor::RocksDb(cursor) => cursor.last_key(),
            _ => Err(Error::MismatchedDatabaseVariant),
        }
    }
    ...
    pub fn next_key(&mut self) -> Result<Option<Key>, Error> {
        match self {
            ...
            #[cfg(feature = "rocksdb")]
            Cursor::RocksDb(cursor) => cursor.next_key(),
            _ => Err(Error::MismatchedDatabaseVariant),
        }
    }
    ...
    pub fn get_current(&mut self) -> Result<Option<(Key, Value)>, Error> {
        match self {
            ...
            #[cfg(feature = "rocksdb")]
            Cursor::RocksDb(cursor) => cursor.get_current(),
            _ => Err(Error::MismatchedDatabaseVariant),
        }
    }
    ...
    pub fn delete_current(&mut self) -> Result<(), Error> {
        match self {
            ...
            #[cfg(feature = "rocksdb")]
            Cursor::RocksDb(cursor) => cursor.delete_current(),
            _ => Err(Error::MismatchedDatabaseVariant),
        }
    }
    ...
    pub fn put<K: AsRef<[u8]>, V: AsRef<[u8]>>(&mut self, key: K, value: V) -> Result<(), Error> {
        match self {
            ...
            #[cfg(feature = "rocksdb")]
            Self::RocksDb(cursor) => cursor.put(key, value),
            _ => Err(Error::MismatchedDatabaseVariant),
        }
    }
}
