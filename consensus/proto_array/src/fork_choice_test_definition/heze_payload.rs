...
pub fn get_heze_chain_following_test_definition() -> ForkChoiceTestDefinition {
    let mut ops = vec![];
    ...
    // its FULL virtual node exists in the Heze fork choice tree.
...
pub fn get_heze_payload_probe_test_definition() -> ForkChoiceTestDefinition {
    let mut ops = vec![];
...
pub fn get_heze_find_head_vote_transition_test_definition() -> ForkChoiceTestDefinition {
    let mut ops = vec![];
...
pub fn get_heze_parent_empty_when_child_points_to_grandparent_test_definition()
-> ForkChoiceTestDefinition {
    let mut ops = vec![];
...
pub fn get_heze_interleaved_attestations_test_definition() -> ForkChoiceTestDefinition {
    let mut ops = vec![];
...
pub fn get_heze_payload_received_interleaving_test_definition() -> ForkChoiceTestDefinition {
    let mut ops = vec![];
...
pub fn get_heze_previous_slot_tiebreaker_test_definition() -> ForkChoiceTestDefinition {
    let mut ops = vec![];
...
pub fn get_heze_proposer_boost_flips_ancestor_test_definition() -> ForkChoiceTestDefinition {
    let mut ops = vec![];
...
mod tests {
    ...
    fn heze_fork_boundary_spec() -> ChainSpec {
        let mut spec = MainnetEthSpec::default_spec();
        spec.proposer_score_boost = Some(50);
    ...
    /// Heze fork boundary: a chain starting pre-Heze (V17 nodes) that crosses into
    /// Heze (V29 nodes). The head should advance through the fork boundary.
    ///
    /// Parameters:
    /// - `skip_first_heze_slot`: if true, there is no block at the first Heze slot (slot 32);
    ///   the first V29 block appears at slot 33.
    /// - `first_heze_block_full`: if true, the first V29 block extends the parent V17 node's
    ///   EL chain (Full parent payload status). If false, it doesn't (Empty).
    fn get_heze_fork_boundary_test_definition(
        skip_first_heze_slot: bool,
        first_heze_block_full: bool,
    ) -> ForkChoiceTestDefinition {
        let mut ops = vec![];

        // Block at slot 31 — last pre-Heze slot. Created as a V17 node because
        // heze_fork_epoch = 1 means Heze starts at slot 32.
        //
        ...
        // First Heze block (V29 node).
        let heze_slot = if skip_first_heze_slot { 33 } else { 32 };

        // The first Heze block should always have the pre-Heze block as its execution parent,
        // although this is currently not checked anywhere (the spec doesn't mention this).
        ops.push(Operation::ProcessBlock {
            slot: Slot::new(heze_slot),
            root: get_root(2),
            parent_root: get_root(1),
            justified_checkpoint: get_checkpoint(0),
            finalized_checkpoint: get_checkpoint(0),
        ...
        ops.push(Operation::ProcessBlock {
            slot: Slot::new(heze_slot + 1),
            root: get_root(3),
            parent_root: get_root(2),
            justified_checkpoint: get_checkpoint(0),
            finalized_checkpoint: get_checkpoint(0),
            execution_payload_parent_hash: if first_heze_block_full {
                Some(get_hash(2))
            } else {
                Some(get_hash(1))
            },
        ...
        ops.push(Operation::FindHead {
            ...
            justified_state_balances: vec![1],
            expected_head: get_root(3),
            current_slot: Slot::new(heze_slot + 1),
            expected_payload_status: None,
        });

        ops.push(Operation::AssertParentPayloadStatus {
            block_root: get_root(3),
            expected_status: if first_heze_block_full {
                PayloadStatus::Full
            } else {
                PayloadStatus::Empty
            },
        ...
        ForkChoiceTestDefinition {
            ...
            finalized_checkpoint: get_checkpoint(0),
            operations: ops,
            // Genesis is V17 (slot 0 < Heze fork slot 32), these are unused for V17.
            execution_payload_parent_hash: None,
            execution_payload_block_hash: None,
            spec: Some(heze_fork_boundary_spec()),
        }
    }

    #[test]
    fn fork_boundary_no_skip_full() {
        get_heze_fork_boundary_test_definition(false, true).run();
    }

    #[test]
    fn fork_boundary_no_skip_empty() {
        get_heze_fork_boundary_test_definition(false, false).run();
    }

    #[test]
    fn fork_boundary_skip_first_heze_slot_full() {
        get_heze_fork_boundary_test_definition(true, true).run();
    }

    #[test]
    fn fork_boundary_skip_first_heze_slot_empty() {
        get_heze_fork_boundary_test_definition(true, false).run();
    }

    #[test]
    fn chain_following() {
        let test = get_heze_chain_following_test_definition();
        test.run();
    }

    #[test]
    fn payload_probe() {
        let test = get_heze_payload_probe_test_definition();
        test.run();
    }

    #[test]
    fn find_head_vote_transition() {
        let test = get_heze_find_head_vote_transition_test_definition();
        test.run();
    }

    #[test]
    fn weight_priority_over_payload_preference() {
        let test = get_heze_weight_priority_over_payload_preference_test_definition();
        test.run();
    }

    #[test]
    fn parent_empty_when_child_points_to_grandparent() {
        let test = get_heze_parent_empty_when_child_points_to_grandparent_test_definition();
        test.run();
    }

    #[test]
    fn interleaved_attestations() {
        let test = get_heze_interleaved_attestations_test_definition();
        test.run();
    }

    #[test]
    fn payload_received_interleaving() {
        let test = get_heze_payload_received_interleaving_test_definition();
        test.run();
    }

    #[test]
    fn previous_slot_tiebreaker() {
        let test = get_heze_previous_slot_tiebreaker_test_definition();
        test.run();
    }

    #[test]
    fn proposer_boost_flips_ancestor() {
        let test = get_heze_proposer_boost_flips_ancestor_test_definition();
        test.run();
    }
    ...
    fn mixed_v17_v29_invalidation() {
        ...
        // V29 block at slot 32 (first Heze slot), child of block 1.
        ops.push(Operation::ProcessBlock {
            slot: Slot::new(32),
            root: get_root(2),
            parent_root: get_root(1),
