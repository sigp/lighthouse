    ...
    E: EthSpec,
{
    ...
    /// Test-only: forcibly override the fork-choice store's finalized view to
    /// `checkpoint`, also bumping the justified, unrealized-justified and
    /// unrealized-finalized checkpoints so the store's invariants are
    /// preserved.
    ///
    /// This is intended for harness consumers (e.g. the EF
    /// `gossip_validation` test handler) that need the store's finalized
    /// view to reflect a specific checkpoint without actually finalizing
    /// through the normal `on_block`/`on_attestation` flow.
    ///
    /// `checkpoint.root` must already exist in `proto_array` (typically
    /// genesis or a previously imported block). `justified_state_root` must
    /// be the state root of the block at `checkpoint.root` and present in
    /// the hot store backing `T`.
    ///
    /// This function deliberately does NOT prune `proto_array` — callers
    /// (such as gossip-validation test cases) may still reference
    /// pre-override blocks.
    pub fn override_finalized_checkpoint_for_testing(
        &mut self,
        checkpoint: Checkpoint,
        justified_state_root: Hash256,
    ) -> Result<(), Error<T::Error>> {
        if !self.proto_array.contains_block(&checkpoint.root) {
            return Err(Error::MissingProtoArrayBlock(checkpoint.root));
        }

        self.fc_store.set_finalized_checkpoint(checkpoint);
        self.fc_store
            .set_justified_checkpoint(checkpoint, justified_state_root)
            .map_err(Error::UnableToSetJustifiedCheckpoint)?;
        self.fc_store
            .set_unrealized_justified_checkpoint(checkpoint, justified_state_root);
        self.fc_store
            .set_unrealized_finalized_checkpoint(checkpoint);

        Ok(())
    }

    /// Test-only: override only the finalized checkpoint root.
    ///
    /// This is for EF gossip vectors that intentionally set a finalized
    /// checkpoint root outside the local proto-array in order to test
    /// not-descended-from-finality rejection paths. It deliberately leaves the
    /// justified checkpoint unchanged because no justified state exists for
    /// the synthetic root.
    pub fn override_finalized_checkpoint_root_for_testing(&mut self, checkpoint: Checkpoint) {
        self.fc_store.set_finalized_checkpoint(checkpoint);
        self.fc_store
            .set_unrealized_finalized_checkpoint(checkpoint);
    }

    /// Test-only: replace fork choice's anchor with `anchor_root`.
    ///
    /// EF networking tests provide preloaded blocks as trusted setup context,
    /// not as gossip messages to be re-imported. Some of those contexts start
    /// from a non-genesis state but only provide the post-state and preloaded
    /// blocks, so test harnesses need to seed fork choice with the vector's
    /// actual anchor root before inserting those trusted blocks.
    pub fn override_anchor_block_for_testing(
        &mut self,
        anchor_root: Hash256,
        ...
        let anchor_checkpoint = Checkpoint {
            epoch: spec.genesis_slot.epoch(E::slots_per_epoch()),
            root: anchor_root,
        };
        let current_epoch_shuffling_id =
            AttestationShufflingId::new(anchor_root, anchor_state, RelativeEpoch::Current)?;
        let next_epoch_shuffling_id =
            AttestationShufflingId::new(anchor_root, anchor_state, RelativeEpoch::Next)?;
        let execution_hash = spec
            .fork_name_at_slot::<E>(spec.genesis_slot)
            .gloas_enabled()
            .then_some(ExecutionBlockHash::zero());

        self.proto_array = ProtoArrayForkChoice::new::<E>(
            self.fc_store.get_current_slot(),
            spec.genesis_slot,
            anchor_state_root,
            anchor_checkpoint,
        ...
        self.fc_store.set_finalized_checkpoint(anchor_checkpoint);
        self.fc_store
            .set_justified_checkpoint(anchor_checkpoint, anchor_state_root)
            .map_err(Error::UnableToSetJustifiedCheckpoint)?;
        self.fc_store
            .set_unrealized_justified_checkpoint(anchor_checkpoint, anchor_state_root);
        self.fc_store
            .set_unrealized_finalized_checkpoint(anchor_checkpoint);
