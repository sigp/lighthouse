...
// `upgrade_to_fulu` gets called multiple times, so this makes
// sure we only print the ascii art once.
static ASCII_INIT: std::sync::Once = std::sync::Once::new();
