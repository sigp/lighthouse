...
/// Verify `Deposit.pubkey` signed `Deposit.signature` for each deposit in batches of `DEPOSIT_SIGNATURE_BATCH_SIZE`.
///
...
#[instrument(name = "is_valid_deposit_signature_batch", skip_all, level = "debug")]
