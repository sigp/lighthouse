...
for (( vc=1; vc<=$VC_COUNT; vc++ )); do
    execute_command_add_PID validator_node_$vc.log ./validator_client.sh $DATADIR/node_$vc http://localhost:$((BN_http_port_base + 1)),http://localhost:$((BN_http_port_base + 2)),http://localhost:$((BN_http_port_base + 1)) $DEBUG_LEVEL
done
