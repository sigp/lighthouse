...
for (( bn=1; bn<=$BN_COUNT; bn++ )); do
    ...
    echo $secret
    execute_command_add_PID beacon_node_$bn.log ./beacon_node.sh $SAS -d $DEBUG_LEVEL $DATADIR/node_$bn $((BN_udp_tcp_base + $bn)) $((BN_http_port_base + $bn)) $((BN_udp_tcp_base + $bn + 100)) http://localhost:$((EL_base_auth_http + $bn)) $secret
done
