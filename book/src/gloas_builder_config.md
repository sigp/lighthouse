...
builders:
  ...
  - enabled: true
    ...
    builder_pubkey: "0xa1b2c3d4..."     # optional — reject a bid not signed by this key
...
| `builder_pubkey` | no | *(none)* | The builder's BLS public key, hex-encoded. If set, a returned bid **not** signed by it is rejected. |
| `auth_data` | no | *(UTF-8 of `url`)* | Opaque authentication data, hex-encoded, agreed with the builder out of band. Signed into the request. Defaults to the UTF-8 bytes of `url`. |

All byte fields (`builder_pubkey`, `auth_data`) are `0x`-prefixed hex strings. All payment values
...
- **`max_execution_payment`** — bounds how much of a builder's (off-chain) execution payment counts
  ...
  payment.
- **`builder_pubkey`** — for a direct builder, if set, the returned bid must be signed by this key
  or it is discarded.
