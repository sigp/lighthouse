...
1. Update the withdrawal credential of validator A to 0x02. You can do this using [Siren](./ui.md) or the [staking launchpad](https://launchpad.ethereum.org/en/). Select:
    - source validator: validator A
    - target validator: validator A
    > Note: After the update, the withdrawal credential type 0x02 cannot be reverted to 0x01, except that the validator exits and make a fresh deposit.
