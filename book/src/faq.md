...
- [Can I submit a voluntary exit message without a beacon node?](#can-i-submit-a-voluntary-exit-message-without-a-beacon-node)
...
- [My beacon node logs `CRIT Beacon block processing error error: ValidatorPubkeyCacheLockTimeout`, what should I do?](#my-beacon-node-logs-crit-beacon-block-processing-error-error-validatorpubkeycachelocktimeout-what-should-i-do)
...
A computer with hardware specifications stated in the [Recommended System Requirements](./installation.md#recommended-system-requirements) can run hundreds validators with only marginal increase in cpu usage. When validators are active, there is a bit of an increase in resources used from validators 0-64, because you end up subscribed to more subnets. After that, the increase in resources plateaus when the number of validators goes from 64 to ~500.
...
### My beacon node logs `CRIT Beacon block processing error error: ValidatorPubkeyCacheLockTimeout`, what should I do? 
