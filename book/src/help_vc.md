...
Flags:
      ...
      --disable-beacon-head-monitor
          Disable the beacon head monitor which triggers attestations and sync
          committee messages when a configured beacon node sends a head event.
          Leaving it enabled is recommended, but disabling it can lead to
          reduced bandwidth and more predictable usage of the primary beacon
          node (rather than the fastest BN).
