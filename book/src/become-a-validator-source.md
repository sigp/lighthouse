...
Replacing the graffiti in quotes with yours, find poap graffiti at [Beaconchain](https://beaconcha.in/poap) then start the validator client with:

```bash
 lighthouse --testnet medalla validator --auto-register --graffiti "poapAH5gxmnPltwyZV0escHrz5ZFmXUB"
```
...
runs you should leave off the `--auto-register` flag. The `--graffiti` flag allows testers to participate
in the Proof of Attendance Protocol for a chance to earn one of the tokens via [merkle drop](https://blog.ricmoo.com/merkle-air-drops-e6406945584d) between genesis and block 300,000 during testnet.
