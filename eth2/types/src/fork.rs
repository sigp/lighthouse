...
impl TreeHash for Fork {
    fn hash_tree_root_internal(&self) -> Vec<u8> {
        ...
        result.append(&mut self.pre_fork_version.hash_tree_root_internal());
        result.append(&mut self.post_fork_version.hash_tree_root_internal());
        result.append(&mut self.fork_slot.hash_tree_root_internal());
        hash(&result)
    }
}
