...
use committees_cache::CommitteesCache;
...
mod committees_cache;

#[derive(Debug, PartialEq)]
pub enum FastBeaconStateError {
    EpochOutOfBounds,
    UnableToShuffle,
...
pub enum AttestationParticipantsError {
    ...
    NoCommitteeForShard,
    FastBeaconStateError(FastBeaconStateError),
}
...
pub struct FastBeaconState {
    ...
    // Cache
    committees_cache: CommitteesCache,
}

impl FastBeaconState {
    ...
    pub fn genesis(
        ...
        spec: &ChainSpec,
    ) -> Result<FastBeaconState, FastBeaconStateError> {
        let initial_crosslink = Crosslink {
            epoch: spec.genesis_epoch,
            shard_block_root: spec.zero_hash,
        };

        let mut genesis_state = FastBeaconState {
            /*
             * Misc
             */
             ...
             * Caches (not in spec)
             */
            committees_cache: CommitteesCache::new(spec.genesis_epoch, spec),
        };
    ...
    /// Return the tree hash root for this `FastBeaconState`.
    ///
    /// Spec v0.2.0
    ...
    pub fn generate_seed(
        ...
        epoch: Epoch,
        spec: &ChainSpec,
    ) -> Result<Hash256, FastBeaconStateError> {
        let mut input = self
            .get_randao_mix(epoch, spec)
            .ok_or_else(|| FastBeaconStateError::InsufficientRandaoMixes)?
            .to_vec();

        input.append(
            &mut self
                .get_active_index_root(epoch, spec)
                .ok_or_else(|| FastBeaconStateError::InsufficientIndexRoots)?
                .to_vec(),
        );
    ...
    pub fn get_crosslink_committees_at_slot(
        ...
        spec: &ChainSpec,
    ) -> Result<Vec<(Vec<usize>, u64)>, FastBeaconStateError> {
        ...
        let (committees_per_epoch, seed, shuffling_epoch, shuffling_start_shard) =
            if epoch == current_epoch {
                ...
                return Err(FastBeaconStateError::EpochOutOfBounds);
            };

        let shuffling = self
            .get_shuffling(seed, shuffling_epoch, spec)
            .ok_or_else(|| FastBeaconStateError::UnableToShuffle)?;
    ...
    pub fn attestation_slot_and_shard_for_validator(
        ...
        spec: &ChainSpec,
    ) -> Result<Option<(Slot, u64, u64)>, FastBeaconStateError> {
        let mut result = None;
    ...
    pub fn get_beacon_proposer_index(
        ...
        slot: Slot,
        spec: &ChainSpec,
    ) -> Result<usize, FastBeaconStateError> {
        ...
        committees
            .first()
            .ok_or(FastBeaconStateError::InsufficientValidators)
...
impl From<FastBeaconStateError> for AttestationParticipantsError {
    fn from(e: FastBeaconStateError) -> AttestationParticipantsError {
        AttestationParticipantsError::FastBeaconStateError(e)
    }
}
...
impl Encodable for FastBeaconState {
...
impl Decodable for FastBeaconState {
...
impl TreeHash for FastBeaconState {
...
impl<T: RngCore> TestRandom<T> for FastBeaconState {
