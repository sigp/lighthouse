...
pub fn run<E: EthSpec>(
    ...
    match &db_manager_config.subcommand {
        ...
        cli::DatabaseManagerSubcommand::Blobs(blobs_manager_cmd) => match blobs_manager_cmd {
            BlobsManager::Verify(verify_config) => env
                .runtime()
                .block_on(verify_blobs::<E>(verify_config, &spec.clone())),
            BlobsManager::Export(export_config) => {
                env.runtime().block_on(export_blobs::<E>(export_config))
            }
            BlobsManager::Import(import_config) => {
                env.runtime().block_on(import_blobs::<E>(import_config))
            }
        },
    }
}
