...
use builder_types::{BuilderConfig, BuilderEntry, RequestAuthData, SignedRequestAuth};
...
impl BuilderStore {
        ...
        E: std::fmt::Debug,
    {
        ...
        let signed = futures::future::join_all(definitions.into_iter().map(|definition| {
            let auth_data = definition
                .auth_data
                .clone()
                .unwrap_or_else(|| definition.url.to_default_auth_data());
            let signing = sign(auth_data);
            async move { (definition, signing.await) }
        }))
        .await;
