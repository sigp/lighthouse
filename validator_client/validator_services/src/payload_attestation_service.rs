...
mod tests {
    ...
    impl TestHarness {
        ...
        async fn create_validators_with_channel(
            num_validators: usize,
            payload_rx: mpsc::Receiver<PayloadAvailableEvent>,
        ) -> Self {
            let mut default_spec = MainnetEthSpec::default_spec();
            default_spec.gloas_fork_epoch = Some(Epoch::new(0));
            let spec = Arc::new(default_spec);
    ...
    async fn poll_for_payload_available_event_filters_stale_events() {
        ...
        let harness = TestHarness::create_validators_with_channel(1, payload_rx).await;
    ...
    #[tokio::test]
    async fn early_event_triggers_publish_before_deadline() {
        let (payload_tx, payload_rx) = mpsc::channel::<PayloadAvailableEvent>(10);
        let mut harness = TestHarness::create_validators_with_channel(1, payload_rx).await;

        // Advance slot clock to slot 1 so the event matches current_slot
        let slot_duration = harness.service.chain_spec.get_slot_duration();
        harness.service.slot_clock.advance_time(slot_duration);
        ...
        // Pre-load the event BEFORE calling run_update.
        // poll_for_payload_available_event finds it immediately and returns,
        // winning the select! against wait_for_attestation_slot's 21-second sleep.
        // No time pausing needed — the channel already has the message.
        payload_tx
            .send(PayloadAvailableEvent {
                beacon_node_index: 0,
                slot: attestation_slot,
        ...
        harness.service.run_update().await;

        // run_update() returns after spawning produce_and_publish as a task.
        // Give it real time to complete its HTTP round-trips against the mock server.
        tokio::time::sleep(Duration::from_millis(500)).await;

        // Attestation was published — proving the early event path fired.
        // The deadline sleep (21 seconds) never had a chance to complete.
        mock_ssz.expect(1).assert();
        mock_json.expect(0).assert();
        ...
        assert_eq!(
            messages.len(),
            1,
            "Payload attestation must be published via early event, not the deadline"
        );
        assert_eq!(messages[0].data.beacon_block_root, expected_block_root);
    }
